...
async function loadStatus() {
  try {
    ...
    isOfflineMode = true;
    document.getElementById('stat-resume-status').innerText = 'Offline/Netlify';
  }
}
...
async function loadJobs() {
  ...
  try {
    ...
    isOfflineMode = true;
    // Load built-in fallback remote job opportunities for static / Netlify mode
    if (allJobs.length === 0) {
      allJobs = getFallbackJobsList();
    }
  }

  // Vector rank client side if offline
...
async function loadResume() {
  ...
  // Fallback candidate profile if offline or resume file not yet created
...
async function saveResume() {
  ...
  // Offline / client-side re-indexing fallback
  allJobs = clientSideVectorRank(allJobs, currentResumeText);
  populateJobDropdowns(allJobs);
  filterJobs();
  ...
  reader.onload = async (e) => {
    try {
      ...
      if (data && data.success) {
        ...
        // Fallback text extraction for client side
...
// Client-side fallback vector matcher (Fast Cosine & Keyword Synergy)
